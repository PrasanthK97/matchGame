...
const fruitList = imagesList.filter(each => each.category === 'FRUIT')
// Replace your code here
class App extends Component {
  state = {
    selectedImg: fruitList[0].imageUrl,
    selectedTab: 'FRUIT',
    mainList: fruitList,
    sec: parseInt(60),
    score: 0,
  ...
  componentDidMount() {
    this.timer = setInterval(this.setInter, 1000)
  }
  ...
  tabSelect = tabId => {
    ...
    const filteredList = imagesList.filter(each => tabId === each.category)
  ...
  imageSelect = url => {
    ...
    this.setState({selectedImg: randomValue.imageUrl})
  }

  reset = () => {
    this.setState({sec: 60, score: 0, isOut: false})
  }
  ...
  render() {
    const {mainList, selectedTab, selectedImg, sec, score, isOut} = this.state

    return (
      <div>
        <Header sec={sec} score={score} />
        <div id="bg">
          {isOut ? (
            this.failure()
          ) : (
            <>
              <div id="main-tile">
                <img alt="match" id="main-img" src={selectedImg} />
              </div>
              <ul id="tab-container">
                {tabsList.map(each => (
                  <li key={each.tabId}>
                    <Tabs tabItem={each} tabSelect={this.tabSelect} />
                  </li>
                ))}
              </ul>
